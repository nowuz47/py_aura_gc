...
int auragc_psi_read(auragc_psi_reading_t *reading) {
    ...
    reading->some_pressure = some_avg10 / 100.0;  /* Convert percentage to 0.0-1.0 */
    reading->full_pressure = full_avg10 / 100.0;
    reading->critical = (some_avg10 >= PSI_CRITICAL_THRESHOLD_MS) || 
                        (full_avg10 >= PSI_CRITICAL_THRESHOLD_MS);
